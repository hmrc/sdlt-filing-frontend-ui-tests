...
import uk.gov.hmrc.ui.pages.{AboutTheTransactionPage, AuthWizard, IndividualOrCompanyPage, InitialPage, IsAnIndividualPage, PurchasersNamePage}
...
class InitialSpec
  ...
  Feature("SDLT Filing frontend Task List Homepage") {
    ...
    Scenario("Hit the TaskList with a return id") {
      ...
      Then("User clicks Prelim Questions Task")
      InitialPage.clickLinkById("task-list-link-prelim-questions")
      Then("User navigates to Individual or Company Name page")
      IndividualOrCompanyPage.navigateToPage(
        "http://localhost:10910/stamp-duty-land-tax-filing/preliminary-questions/who-is-making-the-purchase"
      )
      And("User should be on the Individual or Company Name page")
      IndividualOrCompanyPage.verifyPageTitle(IndividualOrCompanyPage.pageTitle)
